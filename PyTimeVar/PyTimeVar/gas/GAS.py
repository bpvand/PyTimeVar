...
class GAS:
        ...
        if self.if_hetero == False:
            ...
            if self.method == 'gaussian':
                ...
                lhVal =( -0.5 * (dnu + 1) * lhVal + self.n*(gammaln((dnu + 1) / 2) - \
                    gammaln(dnu / 2) - 0.5 * np.log(np.pi * dnu) - np.log(dsigmau)))/self.n
    ...
    def _confidence_bands(self, alpha, iM):
        ...
        vbeta0 = np.linalg.inv((self.mX[:dnInitial, :]).T @ (self.mX[:dnInitial, :])) @ (
            (self.mX[:dnInitial, :]).T @ self.vY[:dnInitial])
        mBetaDraws = np.zeros((iM, self.n-1, self.n_est))
        filt = self._g_filter if self.method == 'gaussian' else self._t_filter
        if self.if_hetero == False:
            for m in range(iM):
                filted=filt(vbeta0, mDraws[m,:])
                mBetaDraws[m,:,:] =  filted
        else:
            for m in range(iM):
                filted,_=filt(vbeta0, mDraws[m,:])
                mBetaDraws[m,:,:] =  filted
    ...
    def plot(self, tau: list = None, confidence_intervals: bool = False, alpha = 0.05, iM = 1000):
        ...
        if self.if_hetero == False:
            if self.n_est == 1:
                ...
                plt.plot(x_vals[tau_index[0]+1:tau_index[1]], self.vY[tau_index[0]+1:tau_index[1]], label="True data", linewidth=1,color='black')
                if self.method=='student':
                    plt.plot(x_vals[tau_index[0]+1:tau_index[1]], self.betas[tau_index[0]:tau_index[1]], label="Estimated $\\beta_{0}$ - tGAS", linestyle="--", linewidth=2)
            ...
            if self.n_est == 1:
                ...
                plt.figure(figsize=(12, 6))
                if self.method=='student':
                    plt.plot(x_vals[tau_index[0]+1:tau_index[1]],self.sigma2_t,label='Estimated $\sigma^2_{u,t}$ - tGAS')
                elif self.method=='gaussian':
                    plt.plot(x_vals[tau_index[0]+1:tau_index[1]],self.sigma2_t,label='Estimated $\sigma^2_{u,t}$ - gGAS')
                plt.grid(linestyle='dashed')
                plt.xlabel('$t/n$',fontsize="xx-large")
