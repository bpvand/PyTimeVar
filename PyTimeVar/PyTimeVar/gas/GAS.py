...
class GAS:
    ...
    def fit(self):
        ...
        vbeta0 = np.linalg.inv((self.mX[:dnInitial, :]).T @ (self.mX[:dnInitial, :])) @ ((self.mX[:dnInitial, :]).T @ self.vY[:dnInitial])
        LB = np.concatenate(([0.001], -10 * np.ones(self.n_est), -np.ones(self.n_est), -50 * np.ones(self.n_est)))
        UB = np.concatenate(([100], 10 * np.ones(self.n_est), np.ones(self.n_est), 50 * np.ones(self.n_est)))
        ...
        if self.method == 'gaussian':  # MLE by Gaussian-GAS
            ...
            vdelta0 = 1
            vtheta0 = np.concatenate([0.1*np.ones(self.n_est),  0.1*np.ones(self.n_est),0.1*np.ones(self.n_est)])
            vgamma0 = np.concatenate([[vdelta0], vtheta0])
        
            fgGAS_lh = lambda vpara: -self.construct_likelihood(vbeta0, vpara)
            ...
            vtheta0 = np.concatenate([np.ones(self.n_est), -0.1 * np.ones(self.n_est), 0.1 * np.ones(self.n_est)])
            vgamma0 = np.concatenate([vdelta0, vtheta0])
            
            ftGAS_lh = lambda vpara: -self.construct_likelihood(vbeta0, vpara)
    ...
    def plot(self, date_range=None):
        ...
        if date_range:
            ...
            x_vals = self._generate_dates(self.betas.shape[0], start_date, end_date)
        else:
            x_vals = np.linspace(0, 1, self.betahat.shape[1])
            
        for i in range(self.n_est):
            axs[i].plot(x_vals, self.betas[:,i], label=r'$\beta_{{{:2d}}}$'.format(i+1))
