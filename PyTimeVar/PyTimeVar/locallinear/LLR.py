...
class LocalLinear:
    ...
    def __init__(
        ...
        if tau is None:
            ...
            if tau[0] <= 0:
                self.tau = np.arange(1 / self.n, (self.n + 1) / self.n, 1 / self.n)[
                           int(0):int(tau[1] * self.n)]

                self.tau_index = np.array([int(0), int(tau[1] * self.n)])
            else:
                self.tau = np.arange(1/self.n,(self.n+1)/self.n,1/self.n)[int(tau[0]*self.n-1):int(tau[1]*self.n)]

                self.tau_index = np.array([int(tau[0]*self.n-1),int(tau[1]*self.n)])
    ...
    def fit(self):
        ...
        if betahat.shape[0] == 1:
            ...
            # Multiple betas
            predicted_y = np.sum(betahat * self.mX[self.tau_index[0]: self.tau_index[1]].T, axis=0)
            
        # store results in corresponding attributes
        self.betahat = betahat
        self.predicted_y = (predicted_y.reshape(-1,1))
        self.residuals = (self.vY)[self.tau_index[0]:self.tau_index[1]] - self.predicted_y
    ...
    def plot_betas(self):
        ...
        """
        x_vals = np.arange(1/self.n,(self.n+1)/self.n,1/self.n)[self.tau_index[0]: self.tau_index[1]]
        
        if self.n_est == 1:
    
            plt.figure(figsize=(12, 6))
            plt.plot(x_vals, self.vY[self.tau_index[0]:self.tau_index[1]], label="True data", linewidth=2,color='black')
