        ...
        if not individual:
            if self.p_dim == 1:
                ...
                for i in range(self.p_dim):
                    ...
                    if self.smooth is not None:
                        plt.plot(x_vals[tau_index[0]:tau_index[1]], self.smooth[tau_index[0]:tau_index[1], i], label=f"Estimated $\\beta_{i}$ - Smoother", linestyle="--", linewidth=2)
                    if self.smooth is not None:
                        plt.plot(x_vals[tau_index[0]+1:tau_index[1]], self.pred[tau_index[0]+1:tau_index[1], i], label=f"Estimated $\\beta_{i}$ - Predictor", linestyle="-", linewidth=2)
                    if self.filt is not None:
                        plt.plot(x_vals, self.filt[tau_index[0]:tau_index[1], i], label=f"Estimated $\\beta_{i}$ - Filter", linestyle="-.", linewidth=2)
            ...
            if self.p_dim == 1:
                ...
                if self.smooth is not None:
                    ...
                    for i in range(self.p_dim):
                        ...
                        if confidence_intervals:
                            vU_bound = self.smooth[:,i][tau_index[0]:tau_index[1]] + st.norm.ppf(1-alpha)*(np.sqrt(self.V[tau_index[0]:tau_index[1],i,i]))
                            vL_bound = self.smooth[:,i][tau_index[0]:tau_index[1]] + st.norm.ppf(alpha)*(np.sqrt(self.V[tau_index[0]:tau_index[1],i,i]))
                            plt.fill_between(x_vals[tau_index[0]:tau_index[1]], vL_bound, vU_bound, label='Pointwise - Smoother', color='grey', alpha=0.3)
                ...
                if self.pred is not None:
                    ...
                    for i in range(self.p_dim):
                        ...
                        if confidence_intervals:
                            vU_bound = self.pred[:,i][tau_index[0]+1:tau_index[1]] + st.norm.ppf(1-alpha)*np.sqrt(self.pred[1+tau_index[0]:tau_index[1],i])
                            vL_bound = self.pred[:,i][tau_index[0]+1:tau_index[1]] + st.norm.ppf(alpha)*np.sqrt(self.pred[1+tau_index[0]:tau_index[1],i])
                            plt.fill_between(x_vals[tau_index[0]+1:tau_index[1]], vL_bound, vU_bound, label='Pointwise - Predictor', color='grey', alpha=0.3)
                ...
                if self.filt is not None:
                    ...
                    for i in range(self.p_dim):
                        plt.subplot(self.p_dim, 1, i + 1)
                        plt.plot(x_vals[tau_index[0]:tau_index[1]], self.filt[tau_index[0]:tau_index[1], i], label=f"Estimated $\\beta_{i}$ - Filter", linestyle="--", linewidth=2)
                        
                        if confidence_intervals:
                            vU_bound = self.filt[:,i][tau_index[0]:tau_index[1]] + st.norm.ppf(1-alpha)*np.sqrt(self.P_filt[tau_index[0]:tau_index[1],0,0])
                            vL_bound = self.filt[:,i][tau_index[0]:tau_index[1]] + st.norm.ppf(alpha)*np.sqrt(self.P_filt[tau_index[0]:tau_index[1],0,0])
                            plt.fill_between(x_vals[tau_index[0]:tau_index[1]], vL_bound, vU_bound, label='Pointwise - Filter', color='grey', alpha=0.3)
