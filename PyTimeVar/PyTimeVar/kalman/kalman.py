...
class Kalman:
    ...
    def __init__(self, vY: np.ndarray=None, T: np.ndarray=None, Z:np.ndarray=None,R:np.ndarray = None, Q:np.ndarray=None, H:np.ndarray=None, a_1:np.ndarray=None, P_1:np.ndarray=None, regressors: np.ndarray= None):
        ...
        if self.Q is None:
            self.bEst_Q = True                                                     # Transition covariance matrix
        self.H = H  
        if self.H is None:
            self.bEst_H = True                                                    # Observation covariance matrix
        self.a_1 = a_1 if a_1 is not None else np.array([[0]])            # Initial state mean
        self.P_1 = P_1 if P_1 is not None else np.array([[1000]])       # Initial state covariance
        self.R = R if R is not None else np.eye(self.a_1.shape[0])         # Transition covariance matrix
    ...
    def _estimate_ML(self):
        if self.bEst_H and self.bEst_Q:
            vH_init, vQ_init = np.ones(self.m_dim**2)*100, np.ones(self.p_dim**2)*100
        ...
        LL_model = minimize(self.compute_likelihood_LL, vTheta, method = 'SLSQP', bounds=((0.00001,None),)*len(vTheta), options={'maxiter':300})
    ...
    def compute_likelihood_LL(self, vTheta):
        if self.bEst_H and self.bEst_Q:
            self.H, self.Q = vTheta[:self.m_dim**2].reshape(self.m_dim, self.m_dim), vTheta[self.m_dim**2:].reshape(self.p_dim, self.p_dim)
        elif self.bEst_H and not self.bEst_Q:
        ...
        for t in range(self.n):
            dLL = dLL - 0.5*(np.linalg.det(F[t,:,:]) + v[t,:,:].T@np.linalg.inv(F[t,:,:]@v[t,:,:]))
    ...
    def fit(self, option):
        ...
        if option.lower() == 'filter':
            ...
            raise ValueError('Unknown option provided to fit(). Choose either filter or smoother')
    ...
    def plot(self):
        """
        Plots the original series and the trend component (filter or smoother).
        """
        if self.filt is None and self.smooth is None:
            print("Model is not fitted yet.")
            return
        
        plt.figure(figsize=(12, 6))
        if self.filt is not None:
            plt.plot(self.filt, label='Kalman filter')
        if self.smooth is not None:
            plt.plot(self.smooth, label='Kalman smoother')
